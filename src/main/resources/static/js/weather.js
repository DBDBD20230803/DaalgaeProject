    ...
    let geoSuccess = function(position) {
        ...
        $.ajax({
            ...
            success: function(data){
                ...
                let makeUrlFindWeather = "https://apis.data.go.kr/1360000/VilageFcstInfoService_2.0/getVilageFcst?serviceKey=jwzbFiirkPF%2FrzTy3ceQqXb%2BuP8bvOFMVH3CizfiVQsy5EDFZCZekZa4S63aD460NnsTzQP4Ua5sfb5yT9stZg%3D%3D&pageNo=1&numOfRows=300&dataType=JSON&base_date=";
                makeUrlFindWeather += date1;
                makeUrlFindWeather += "&base_time=2000&nx=";
                makeUrlFindWeather += weatherX;
                makeUrlFindWeather += "&ny=";
                makeUrlFindWeather += weatherY;
                $.ajax({
                    type:"get",
                    url:makeUrlFindWeather,
                    dataType:"json",
                    success: function(data){
                        console.log(makeUrlFindWeather);
                        console.log(data);
                        console.log("통신성공2");
                        // console.log(data);
                        ...
                        let weatherLevel = 0;
                        let tempCheck = 0;
                        Outer: for(let a = 0; a < 300; a++) {
                            let itemSelect = data.response.body.items.item[a];
                            console.log(date1);
                            console.log(a);
                            if(itemSelect.fcstDate == date1) {
                                console.log(itemSelect.category);
                                if(itemSelect.category == 'TMP') {
                                    if(tempCheck == 0) {
                                        $('.weatherTemp0').append("온도 " + itemSelect.fcstValue + "℃ | ");
                                        $('.weatherTemp1').append("온도 " + itemSelect.fcstValue + "℃ | ");
                                        $('.weatherTemp2').append("온도 " + itemSelect.fcstValue + "℃ | ");
                                        tempCheck = 1;
                                    }
                                    if(itemSelect.fcstValue >= 28) {
                                        if(weatherLevel < 3) {
                                            weatherNo = 1;
                                            weatherLevel = 3;
                                        }
                                    }
                                    if(itemSelect.fcstValue <= 10) {
                                        if(weatherLevel < 3) {
                                            weatherNo = 2;
                                            weatherLevel = 3;
                                        }
                                ...
                                // if(itemSelect.category === 'POP') {
                                //     if(itemSelect.fcstValue >= 70) {
                                ...
                                //     }
                                // }
                                if(itemSelect.category == 'PCP') {
                                    if(itemSelect.fcstValue >= 20) {
                                        if(weatherLevel < 3) {
                                            weatherNo = 3;
                                            weatherLevel = 3;
                                        }
                                ...
                                if(itemSelect.category == 'PTY') {
                                    if(itemSelect.fcstValue == 1 || itemSelect.fcstValue == 2) {
                                        if(weatherLevel < 3) {
                                            weatherNo = 4;
                                            weatherLevel = 3;
                                        }
                        ...
                        // for(let a of b) {
                        //     if(a.fcstTime === hourString && a.category === 'POP') {
                        //         alert(a.fcstValue);
                        //     }
                        // }
                        // console.log(data.response.body.items.item[1].fcstValue);

                        let makeurlFindDust = "https://apis.data.go.kr/B552584/ArpltnInforInqireSvc/getMinuDustFrcstDspth?serviceKey=jwzbFiirkPF%2FrzTy3ceQqXb%2BuP8bvOFMVH3CizfiVQsy5EDFZCZekZa4S63aD460NnsTzQP4Ua5sfb5yT9stZg%3D%3D&returnType=json&numOfRows=500&pageNo=1&searchDate=";
                        makeurlFindDust += date2;
                        makeurlFindDust += "&InformCode=PM10";
                        $.ajax({
                            type:"get",
                            url:makeurlFindDust,
                            dataType:"json",
                            success: function(data){
                                ...
                                for(let b of a) {
                                    if(b.substring(0, 2) === addressLevel1.substring(0, 2)) {
                                        let c = b.indexOf(":");
                                        $('.weatherTemp0').append("미세먼지 " + b.substring(c+2));
                                        $('.weatherTemp1').append("미세먼지 " + b.substring(c+2));
                                        $('.weatherTemp2').append("미세먼지 " + b.substring(c+2));
                                        break;
                                    }
                                }
                            },
                ...
                // 주소 정보(주소 레벨1의 두글자로) 미세 먼지 정보 찾기(좋음, 보통, 나쁨 이런 거)
