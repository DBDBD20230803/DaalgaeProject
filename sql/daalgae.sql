...
--   권한코드입니다. 'ROLE_MEMBER', 'ROLE_ADMIN' 이 있습니다.
    `MEM_BAN_STATUS`    VARCHAR(2) DEFAULT 'N' NOT NULL,
...
(
    ...
    `POST_SORT` VARCHAR(30) NOT NULL CHECK(POST_SORT IN ('공지', '이벤트', '자유', '먹거리', '용품', '정보', '웹툰')),
...
(
    ...
    `REF_USER_BAN_CODE`    INTEGER NOT NULL,
...
ALTER TABLE ATTACHMENT_FILE ADD FOREIGN KEY (REF_POST_CODE) REFERENCES POST_INFO(POST_CODE);
...
ALTER TABLE POST_REPLY ADD FOREIGN KEY (REF_POST_CODE) REFERENCES POST_INFO(POST_CODE);
ALTER TABLE POST_REPLY ADD FOREIGN KEY (REF_POST_REPLY_WRITER) REFERENCES MEM_INFO(MEM_CODE);

ALTER TABLE ENCYCLE_SORT ADD FOREIGN KEY (REF_MEM_CODE,REF_MEM_BOOKMARK_SEQ) REFERENCES BOOKMARK_INFO(MEM_BOOKMARK_SEQ, REF_MEM_CODE);
